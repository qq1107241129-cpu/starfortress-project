...
- 修复内容：
  ...
  - 修复局内塔等级读取错误（不再从存档读取，固定从 1 级开始）
  - 合金显示最小修复（alloyLabel 未绑定时只 warn）
  - 增强放塔失败日志
- `.scene` 状态：未修改
- 需要用户在 Cocos 中临时绑定：将 DebugInfoLabel 拖到 BattleUI.alloyLabel（可选）
...
- `.meta` 状态：无新增脚本
- 合金是否写入存档：否
- 是否改变存档结构：否（towerLevels 字段名保留，语义变更仅在注释和文档）
- 代码已实现，Web 预览待用户确认

**Cocos 人工操作**：
1. 如果需要显示合金数量，在 BattleUI 组件中绑定 alloyLabel（可选）
2. 如果不绑定，合金数量会通过控制台日志输出
3. 不强制要求修改 .scene
